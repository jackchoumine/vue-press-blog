...
const prototypeAccessors = { namespaced: { configurable: true } }
...
const Store = function Store(options) {
  ...
  const useDevtools = options.devtools !== void 0 ? options.devtools : Vue.config.devtools
  if (useDevtools) {
    devtoolPlugin(this)
  }
}

const prototypeAccessors$1 = { state: { configurable: true } }
...
Store.prototype.dispatch = function dispatch(_type, _payload) {
  ...
  const promises = entry.map(function (handler) {
    return handler(payload)
  })

  const result = entry.length > 1 ? Promise.all(promises) : entry[0](payload)
