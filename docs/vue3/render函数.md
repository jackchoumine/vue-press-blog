...
> 使用 render + setup 函数，2.x 的 data + render 类似。
...
> 可依次判断 key 是否以`on`且值为非函数。

2. 第一种方法还是不能很好区分 dom 属性。

当组件绑定一个 `onName="func"` 是，无法知道这是事件还是传递方法。
...
attrs 里有有一个 onProp 属性，乍一看，以为是事件。
...
> `数据驱动`是 vue、react、angular 等前端框架的灵魂，正式数据驱动的前端才告别手动操作 DOM 的脏活累活。
...
> 上面的用法结合插槽，二次封装开源组件，极为强大且优雅，只需要关注数据，减少维护模板。数据改变，要是需要修改 js，又去修改模板，这是不好的设计。
