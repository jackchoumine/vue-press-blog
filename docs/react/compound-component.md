...
> 控制反转：程序如何工作，不是由开发者控制，而是程序调用者有更多控制权，。可简单理解为程序能实现高度定制化，用户很实现他想要的很多需求。**程序开发者做得很少，但是提供接口给程序调用者实现很多需求**。
...
1. 不够灵活：使用了操作了孩子组件的方法，限制了 Select 的直接子组件只能是 Option，想要使用 div 包裹 Option ，无法实现。
