...
**闭包在外层函数执行时候创建，访问外层函数的作用域使使用闭包的目的。**
...
> outer 调用时创建了一个闭包，能记住它的作用域，在 count 函数调用时，还是能访问它参数和局部变量。
...
向右边的部分应用，和函数的默认一直，不用改写函数，就能对函数进行部分应用。
...
向右的部分应用不用改写原函数，能充分利用函数的默认参数，向左的这不能，默认情况下都是向右的。
...
对函数多次部分应用，得到的新函数的参数越来越少，一次性对函数的所有参数部分应用，也要再调用一次函数才能得到结果。

对函数的**所有参数**部分应用，也要再调用一次函数才能得到结果。
...
curry(
  setTimeout,
  2 // 不传递的化，会是 5 TODO 为何是 5 ？ 没查到相关资料
)(() => {
  console.log('1000毫秒')
})(1000)
```
...
由于柯里化层层嵌套，当参数很多，恰套会很深，频繁执行的函数，明显比不柯里化的函数慢。
:::
