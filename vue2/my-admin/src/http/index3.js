 ...
 * @Author: JackChou
 * @LastEditTime: 2022-11-16 02:25:06 +0800
...
function addPendingRequest(config) {
  ...
  config.cancelToken = new axios.CancelToken(cancel => {
    ...
    // NOTE 在线才记录重复，允许用户多次尝试
    window.navigator.onLine && refeatRequestsMap[requestKey].push(cancel)
  })
  return config
}
...
function cancelRepeatRequest(config) {
  ...
  if (needCancel) {
    // 不改变原数组
    refeatRequestsMap[requestKey].slice(1).forEach(cancel => {
      cancel(requestKey)
    })
  }
  return config
...
const beforeRequest = config => {
  ...
  // NOTE 取消后来的重复请求
  const __config = cancelRepeatRequest(_config)
  return __config
}
