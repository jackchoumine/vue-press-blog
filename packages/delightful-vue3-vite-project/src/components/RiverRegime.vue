...
const onLoadAllRiverTable = (page, adcd, motype, sttypes, searchKey, defineArea) => {
  ...
  store.api
    .from('water.river.all.table.data', apiParam)
    .then((res: any) => {
      if (res) {
...
const onLoadWarningRiverTable = (
  ...
  const apiParam = { ...currentFilter, ...param }
  store.api
    .from('water.river.warning.table.data', apiParam)
    .then((res: any) => {
      if (res) {
        page.page = res.pageNow
        page.size = res.pageSize
...
//超起淹没列表
const onLoadInitiationRiverTable = (
  page,
  adcd,
  motype,
  sttypes,
...
//   alarmData.alarmTotal = 0
//   alarmData.beyondHistoryMaxHeight = 0
//   alarmData.beyondHistoryMaxHeight = 0
//   alarmData.beyondAlarmWaterHeight = 0
//   alarmData.beyondDrownWaterHeight = 0
//   loading.value = true
//   let param = { adcd: adcd, start: startTime, end: navData.endTime, sttypes: sttypes }
//   const apiParam = { ...currentFilter, ...param }
//   store.api
//     .from('water.river.data', apiParam)
//     .then((res: any) => {
//       if (res) {
//         alarmData.alarmTotal = res.total
//         alarmData.beyondHistoryMaxHeight = res.overHistoryMax
//         alarmData.beyondHistoryMaxHeight = res.overGraz
//         alarmData.beyondAlarmWaterHeight = res.overWarnz
//         alarmData.beyondDrownWaterHeight = res.overInundate
//       }
//     })
//     .catch((error: any) => {
...
const rowClick = (e, target) => {
  ...
  useFlayTo(coords, {
    ...
    marker: {
      ...
      image: `/images/layer-2d/${target.motype}/${
        target.wlevel || target.level || 0
      }.png`,
    },
  })
}
...
<template>
  <div class="module module-river super-alarm-river">
    ...
    <FormTable :col-list="colList" :data="alarmData" />
    ...
    <div class="super-alarm-bottom">
      ...
      <q-tab-panels v-model="navData.tab" animated class="fill-90">
        <q-tab-panel name="1">
          ...
          <ec-table
            v-loading="loading"
            class="header-table table-height"
            :dimensions="riverAllColumns"
            :pagination="page"
            :source="sourceAll"
            @update:page="onAction"
            @table:click="rowClick"></ec-table>
        </q-tab-panel>
        <q-tab-panel name="2">
          <div style="display: flex; justify-content: space-between">
            <div class="search-content">
          ...
          <ec-table
            v-loading="loading"
            class="header-table table-height"
            :dimensions="riverWarningColumns"
            :pagination="page"
            :source="sourceAll"
            @update:page="onAction"
            @table:click="rowClick"></ec-table>
