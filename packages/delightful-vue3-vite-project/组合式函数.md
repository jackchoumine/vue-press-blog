...
```js
const { useHookA, useHookB } = './hooks' // 从 hooks/index.js 导出 hooks
...
> 了解状态之间的逻辑关系，提取组合函数时尤其重要，实际上这对开发者提出了更高的要求：要求开发者有良好的代码设计意识和对业务有比较全面深入的理解。
...
react hook 有严格的调用时序，不能条件语句中调用，vue composition api 没有严格大调用顺序，可在条件语句中调用。
...
> 相比之下，vue composition api 更加灵活强大。
